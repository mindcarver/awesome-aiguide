...
交付标准操作流程（Standard Operating Procedure，简称SOP）是整个商业化体系中最为硬核的环节。真正决定口碑、复购率和持续经营能力的，是交付能力。一个方案写得再漂亮，如果交付质量不行，客户不仅不会续费，还可能在行业里传播负面评价。反过来，一个扎实的交付不仅能带来满意的客户，还能产生案例、口碑和转介绍，这是一个人做事最宝贵的长期资产。

交付 SOP 的本质是把项目执行经验提炼成一套可重复、可预测、可优化的标准流程。有了这套 SOP，每个项目的执行就不再是从零开始摸索，而是按照既定流程高效推进。本篇覆盖从获客到续费的完整交付链路。
...
访谈环节的SOP在前一篇中已经详细讲解过，这里补充一些操作层面的规范。每次访谈之前，你需要完成以下准备工作：查阅客户的背景简报、准备针对性的访谈问题、确认访谈的时间和参与人员、准备好记录工具。访谈过程中，你需要按照既定的访谈框架推进，同时保持灵活性，根据客户的回答调整提问方向。访谈结束后二十四小时内完成会议纪要并发送给客户确认。
...
机会评分环节的SOP。在流程拆解的基础上，对识别出的所有AI应用机会进行评分和排序。评分使用前面设计的痛点评分表，从业务影响度、AI适配度、实施难度和ROI预估四个维度进行评估。评分完成后，选出排名前二到三个机会作为Demo制作的候选场景，与客户确认。
