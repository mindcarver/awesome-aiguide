...
![工具调用经过七道关卡，从模型调用到会话日志](imgs/13-01-flowchart-seven-gates.png)
...
![机制和策略分离：工具保持简单，策略挂在固定管线上](imgs/13-02-comparison-policy-separation.png)
...
![执行前依次经过前置策略门、一次性审批与守卫](imgs/13-03-flowchart-pre-execution-gates.png)
...
![审批 fail-closed 与守卫只拒不放的边界](imgs/13-04-comparison-approval-guard.png)

## 执行与处置：怎么跑，结果谁说了算

![执行包装包住工具本体，结果处置不能改变成败](imgs/13-05-framework-execution-result.png)
...
![归一化将策略、包装和工具的异常收敛为结构化失败结果](imgs/13-06-infographic-normalization.png)
...
![调用先记日志、结果终检冻结后统一分发的两条时序](imgs/13-07-timeline-truthfulness.png)

## 并发：策略有序，执行重叠

![策略按提交顺序决策、执行阶段重叠、结果按原顺序提交](imgs/13-08-flowchart-concurrency-segments.png)
...
![读文件调用可并发，写文件调用独占并排干在跑调用](imgs/13-09-timeline-call-scheduling.png)
...
![Code Mode 与文件接缝复用同一管线，直调工具不构成后门](imgs/13-10-framework-no-backdoor.png)
