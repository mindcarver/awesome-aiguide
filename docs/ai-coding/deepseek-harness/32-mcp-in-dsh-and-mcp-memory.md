...
![MCP 把 N 乘 M 适配变成 N 加 M](imgs/32-01-n-plus-m.webp)
...
![一个桥接插件实例对应一个服务器](imgs/32-02-one-instance-one-server.webp)
...
![MCP 公开工具的稳定命名](imgs/32-03-stable-tool-names.webp)
...
![工具注册的两阶段代际切换](imgs/32-04-two-phase-generation.webp)

![同步队列与代际栅栏](imgs/32-05-sync-queue-fence.webp)
...
![MCP 服务器重连的预算退避](imgs/32-06-reconnect-budget.webp)
...
![MCP 外部内容的防御性解析](imgs/32-07-defensive-content.webp)

![stdio 环境清洗与 HTTP 凭证边界](imgs/32-08-env-and-headers.webp)
...
![一次 MCP 工具调用的完整往返](imgs/32-09-tool-call-round-trip.webp)
...
![保留旧代或回滚归零的两条时间线](imgs/32-10-keep-old-or-zero.webp)
...
![stdio 与 HTTP 的信任账](imgs/32-11-transport-trust-ledger.webp)
...
![mcp-memory 的三份参考配置](imgs/32-12-mcp-memory-configs.webp)
...
![跨会话验证记忆的三步闭环](imgs/32-13-memory-verification.webp)
...
![MCP 桥接的边界与工程纪律](imgs/32-14-mcp-boundaries-summary.webp)
