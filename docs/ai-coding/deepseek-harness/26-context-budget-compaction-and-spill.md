...
![Compaction 由 harness 被动驱动而不是模型工具](imgs/26-01-no-compact-tool.png)

![请求前压力与请求失败后超限构成两种压缩触发](imgs/26-02-passive-triggers.png)
...
![超大工具结果先经过确定性修剪和重测量](imgs/26-03-trim-then-summarize.png)
...
![修剪保留头尾并按 Unicode 码点避免切坏 emoji](imgs/26-04-head-middle-tail-trimming.png)
...
![Token Meter 从持久日志重放并提供可复用锚点](imgs/26-05-token-meter-snapshot.png)

## 阈值与保留尾巴：数字和政策

![上下文阈值与保留尾巴配合逐 step 检查](imgs/26-06-threshold-tail-step-check.png)

![尚未关闭的开放尾 step 没有平衡边界就暂不压缩](imgs/26-07-open-tail-step.png)
...
![start、summary、end 三类压缩日志事件共同形成锁](imgs/26-08-compaction-event-lock.png)

![孤儿压缩锁是否有效取决于最新会话种子边界](imgs/26-09-orphan-lock-seed.png)
...
![总结调用逐字回放前缀并将压缩指令放在尾部以复用 KV 缓存](imgs/26-10-summary-kv-cache.png)

![自动头部压缩追求缓存命中，手动中段压缩优先正确性](imgs/26-11-cache-reuse-boundaries.png)
...
![压缩区域以表面位置为准且不得切断工具调用和结果配对](imgs/26-12-tool-pair-balanced-region.png)
...
![溢出救场只在表面确实推进时才重试](imgs/26-13-surface-progress-retry.png)
...
![很多中等内容用 Compaction，单个超大内容用 Spill](imgs/26-14-compaction-vs-spill.png)
...
![spillStore 只持久化内容并返回不透明定位符与检索提示](imgs/26-15-spill-store-seam.png)

## 本地后端：session 级私有文件

![本地 Spill 以私有会话目录和独占写入保护大文本](imgs/26-16-local-spill-security.png)
...
![Spill 策略在结果规范化前判断超大结果并外置全文](imgs/26-17-spill-policy-waterfall.png)

![read 的模型上下文溢出规则与 code dispatch 日志规则不同](imgs/26-18-read-loop-and-dispatch.png)
...
![Fork 继承 Spill 定位符引用而不复制底层工件](imgs/26-19-fork-locator-inheritance.png)
...
![Token Meter、Compaction 与 Spill 共同守住上下文预算](imgs/26-20-context-budget-summary.png)
