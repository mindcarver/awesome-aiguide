...
![grep 的文本命中与 LSP 语义导航的差异](imgs/22-01-grep-vs-semantic-navigation.png)
...
![极薄的 LSP 接缝将协议复杂度压入 Provider](imgs/22-02-lsp-seam-hides-protocol.png)

## 四个操作，封闭联合

![四个统一的 LSP 语义导航操作构成封闭联合](imgs/22-03-four-closed-operations.png)
...
![按扩展名选择语言服务 Provider 的稳定路由](imgs/22-06-extension-routing.png)
...
![Provider 注册在冲突时整体回滚而不留下半状态](imgs/22-07-atomic-registration.png)
...
![语言服务器通过显式配置按首次查询懒启动](imgs/22-08-explicit-server-deployment.png)

## provider 拿到的是加过工的请求

![完整请求进入接缝后派生语言 ID](imgs/22-09-complete-derived-request.png)
...
![模型友好的一基坐标在工具层转换为零基 UTF-16](imgs/22-10-coordinate-conversion.png)
...
![规范工作区 URI 区分工作区内外的导航位置](imgs/22-11-canonical-workspace-uri.png)

## 一个查询的一生

![一次 LSP 查询从工具调用到归一化结果的生命周期](imgs/22-12-query-lifecycle.png)
...
![瞬态打开文档避免长期同步状态机](imgs/22-13-transient-document-sync.png)
...
![同工作区串行而不同工作区并行的实例池](imgs/22-14-instance-pool-serialization.png)
...
![结果联合与稳定错误码共同组成封闭词表](imgs/22-15-results-and-errors.png)
...
![可靠 LSP 接缝的四项核心价值](imgs/22-16-lsp-seam-summary.png)
