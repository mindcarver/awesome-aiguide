...
![源图被规范化并按 provider 路由派生为模型请求版本](imgs/17-01-image-forms.webp)
...
![会话日志保留内容寻址引用，附件存储保留持久字节](imgs/17-02-durable-reference.webp)
...
![源图通过准入限制后被转正、去元数据并规范化](imgs/17-03-admission-normalization.webp)
...
![内容寻址对象先耐久落盘，之后才发布会话事件](imgs/17-04-content-addressing.webp)
...
![read image 走统一附件生命周期，并报告缩放坐标关系](imgs/17-05-read-image.webp)
...
![input modalities 声明在上传、工具与发送侧分别把关](imgs/17-06-modality-gates.webp)
...
![纯文本模型以确定性占位文本投影历史图片而不改历史](imgs/17-07-text-projection.webp)
...
![规范附件与路由策略确定性派生并缓存请求版本](imgs/17-08-request-variant.webp)
...
![DeepSeek Files API 上传并复用 file id，按有效期和失效状态管理](imgs/17-09-files-api.webp)
...
![Files 失败后复用请求版本走 base64 有界内联回退](imgs/17-10-inline-fallback.webp)

## 装不下：确定性降级

![请求超限时按最旧优先、量子步长省略图片](imgs/17-11-deterministic-degradation.webp)
...
![消息级限额保护持久历史，请求级限额重算临时投影](imgs/17-12-two-limits.webp)
