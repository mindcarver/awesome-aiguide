...
![agent loop 通过 ctx.llm 与中立 StreamChunk 隔离所有 provider](imgs/16-01-framework-adapter-boundary.webp)
...
![provider 字符串在注册表路由到适配器，model 不参与路由](imgs/16-02-flowchart-provider-routing.webp)
...
![llm stream waterfall 用洋葱中间件在适配器之前拦截请求](imgs/16-03-framework-stream-waterfall.webp)
...
![请求、SSE、记账与错误差异均由适配器内部吸收](imgs/16-04-framework-adapter-responsibilities.webp)
...
![空闲看门狗只在流读取挂起时计时](imgs/16-05-timeline-idle-watchdog.webp)

## 回程：一条封闭的流式协议

![流式动作封闭而内容块类型可扩展](imgs/16-06-comparison-protocol-vocabulary.webp)
...
![增量实时流动，在 DONE 后按 block end、usage、finish 收尾](imgs/16-07-flowchart-stream-ordering.webp)
...
![编译期、运行期与消费端共同防御 stream 契约](imgs/16-08-framework-contract-defense.webp)

## 消费：双轨记账，一份拼装实现

![每条 chunk 同时写入日志并交给 BlockAssembler 拼装](imgs/16-09-flowchart-dual-track-consumption.webp)
...
![中断时只保留安全的文本与推理前缀，丢弃工具调用](imgs/16-10-comparison-interruption-safety.webp)

## 失败：两条出口，一种形状

![throw 与流内错误在适配器边界归一，并由策略层决定重试](imgs/16-11-flowchart-failure-retry.webp)
...
![会话日志可离线重放中立 chunk，私有 replayState 不跨适配器](imgs/16-12-framework-replay-boundary.webp)
