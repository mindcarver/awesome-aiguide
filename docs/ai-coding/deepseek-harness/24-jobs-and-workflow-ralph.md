...
> 2026 年 8 月给这套栈补上了两块此前缺失的闭环：任务完成现在能唤醒空闲的 agent（有界、可关、可审计），workflow 运行的成员历史落进会话日志、跨刷新可重建。提示词里"任务完成时你会在会话内收到通知，不要忙轮询"从愿景变成了机制。
...
`ctx.jobs` 里的每个活有一个 `JobId`，格式是 `<kind>-N`，比如 `bash-3`、`subagent-1`。kind 来自一个可合并扩展的表，截至 2026-08 有 `bash` 和 `subagent` 两个，注册表把每个 kind 当成不透明的 id 命名空间。
...
通知的次序有讲究：完成是最后才宣布的。`settle()` 先提交记录、释放等待者、发布可见集变更，全都完成之后，完成监听器才被调用。原因在 2026 年 8 月的一次顺序修正里说得透彻：会开启轮次的报告者是同步执行的，如果宣布排在前面，被唤醒轮次的 `turn/start` 会抢在它所响应的那次结算落账之前出现，报告者看到的还是没提交的状态。把完成放到最后，报告者成为该结算的最后一个观察者，其他观察者都已先看到它。

等待和读取也被这套账本管理。`wait` 用一个正的有限毫秒数设界，模型不传就用默认 30 秒，单次等待有硬上限 10 分钟，更大的值被夹下来。超时只报告"还在跑"，不会顺手把任务停掉。这里有个容易被通用策略误伤的点：这个超时被刻意排除在通用工具超时策略之外，否则通用策略会把一次正常的等待状态替换成超时错误。等到任务后来结算了，被压制的通知照样送达，终态快照赢。

快照上还有一个 `reported` 标志，现在有四个置位方：kill、读、等、销毁。前三者一旦已经报告或承诺报告终态，标志就置位，后续不再重复通知；2026 年 8 月起销毁也置位，因为 owner 正在销毁意味着没有读者了，而会唤醒的报告方若不置位，会把通知变成每个 teardown 层级一次模型请求，作用在宿主正要销毁的 agent 上。置位只否决通知报告方，不否决 `onJobDone` 本身，运行时不变量和强制失败路径依旧被覆盖。
...
修法按所有者当时在做什么选通道。繁忙的所有者走注入，保持原样：只要 next-step inbox 还有内容，轮次就无法结束，通知延长当前轮次，同时结算的多个任务只花一步而不是各占一轮；轮次已取消但未收敛的情形也走注入，因为替一次用户中断重新开轮，等于把中断洗成一次用户没要求的模型请求。空闲的所有者用 `followup()` 唤醒：模型已经说完话、agent 已 idle，完成的任务把 agent 拉回一轮，接着干下去。一小时的构建结束、agent 自动继续，这条链路终于通到了提示词承诺的样子。

唤醒是有界的，而且这个界不是时间。`maxConsecutiveWakes` 默认 3，限制一个所有者由此开启的轮数，超出后通知降级为注入等待下一轮；领取任何用户撰写的消息都会恢复预算，注意是领取而非抵达，那才是人类输入真正进入某一步的时刻，插件自己排队的通知不补充预算。设界是因为这条链会自激：被唤醒的一轮可能又启动某个后台任务，它的完成又唤醒同一个所有者，无人旁观时就是无限循环。想要旧行为的部署设 `completionDelivery: 'quiet'`，通知回到纯注入。这个交付选择被拒掉的替代方案也有账：给 `JobStart` 加生产方声明的唤醒位（对应其他产品的 trigger_turn 枚举）被推迟到第一个需要区分的生产方出现；通用非请求输入队列被否，因为 inbox 本身就是那个队列。
...
owner 的第一个任务会给 `owner.ctx` 挂一个异步 effect，`AgentHandle.dispose()` 要等拥有的后台工作停下来才 resolve。这带来一条容易被忽略的行为变化：后台 bash 跟着它的 agent 一起停，而不是活过它。想让一个活活得比 agent 久，必须以无主身份启动，无主的活对非 agent 调用方开放，随服务销毁而死。
...
引擎级的两个旋钮，`subagentProvider` 和 `maxTotalAgents`，由部署配置决定，脚本观察不到也换不掉。模型拿不到路由选择权，这是后面 Ralph 能站得住的前提。`meta.phases` 只是进度词汇：`phase()` 调用匹配标题给观察者看，不蕴含任何执行结构，UI 也被明文禁止从脚本文本推断静态工作流图，因为只有成员 start 事件能证明工作真正发生。

脚本可用性上还有一个和 CC 不同的松绑：CC 为了可重放禁用脚本里的时钟和随机数，dsh 暂时不禁，脚本能正常拿时间戳和随机数做指数退避或采样。这个自由不是白来的，它依赖的前提是 meta 已经从脚本文本挪进了参数，身份信息不需要执行就能拿到；等 journal 和断点续跑真的做起来，确定性要求会跟着回来。
...
写入有前缀纪律：第一次会话追加失败后，本运行停用所有后续写入、只记一次告警，绝不动取消、结果映射或 dispose。每种失败位置留下的要么是空记录、要么是一段合法的连续前缀：已开始的运行可以缺后续成员或缺终点，已开始的成员也可以缺自己的终点。配套的不变量检查在冷加载和实时追加时兜底：一次 run 恰一个 start、成员序号唯一且为正、成员结尾成对、没有 run 在成员未关时结束、结束之后没有更新。缺了尾部结尾按"中断证据"处理而不是"损坏"，因为强制终止本来就可能截断在中间。

Web 端的呈现由一个专门的对话节点定义和按 run id keyed 的渲染器负责，从运行中到终态保持同一个父级：运行是一行带内联状态点的平台行，阶段是可展开的 disclosure 行，成员行带状态点。阶段只在成员真正开始时出现，按精确阶段字符串分组；成员结算只改状态，不删除不重排。点到子会话的导航不写持久记录，从两个当前权威派生：持久成员状态仍是运行中，且当前会话列表里同一 id 还在跑，行才可交互；终态或远程的成员保持可见但静态。原始的工具调用行原样保留，workflow 节点是新增的观察面，不夺工具卡的所有权。
...
交接报告带状态（continue、complete、blocked）、非空摘要、证据、后续步骤、阻塞说明，序列化上限 16384 字符，在工作流内和消费边界各校验一次。无效、缺失或超限的报告让整个 workflow 失败，而不是被截断或误判成轮数耗尽。这条硬线防的是一个静默腐烂的循环：一份被偷偷截短的报告会让下一轮在残缺状态上继续空转。成功轮的父向终局文本同样有 16384 字符的上限。
...
唤醒机制有自激防线但也有残余风险。耗尽唤醒预算的无人值守 agent，要等到其他原因开启轮次时才收走剩余通知，期间没有机制为它重新充电；`quiet` 模式下待领于空闲所有者的通知仍随所有者释放而消亡。还有一个已知的微任务窗口：结算若落在轮次循环最后一次检查 inbox 之后、driver 提交 idle 相位之前，读到的还是运行中，于是走注入且无人唤醒，堵上它需要核心 agent 层发布退休状态，不属于交付策略的管辖。

一个已知后果：`cancel` 返回了却不结算 `done` 的生产者会堵住销毁，运行时无法把它和一次合法的慢停区分开。防住它的办法在契约里（cancel 必须最终结算 done），不在运行时。

workflow 一个 context 只有一个引擎，没有命名 provider 注册表，换引擎靠插件配置替换。想同时跑两种编排语义不行，这是用"替换"换"语义一致"。每次 run 付一次 worker 启动和 RPC 成本；脚本的中间值不落会话日志，持久化的四种事件记录的是成员边界和终局，重放不能重建脚本内部的中间结果。
...
- [后台任务完成唤醒空闲所有者决策](https://github.com/deepseek-ai/deepseek-harness/blob/master/.agents/notes/implemented/feature/2026-08-11-background-job-completion-wakes-an-idle-owner.zh.md)：交付通道分流、唤醒预算、reported 的第四个置位方
- [Chat 中持久工作流运行决策](https://github.com/deepseek-ai/deepseek-harness/blob/master/.agents/notes/implemented/feature/2026-08-10-durable-workflow-runs-in-chat.zh.md)：四种持久事件、前缀纪律、UI 呈现
...
上一篇：[dsh 的 Code Runtime 与 Code Mode：让模型写代码并执行](./23-code-runtime-and-code-mode.md)
